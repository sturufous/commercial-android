...
export class Pretrip3Page {
  ...
  getPretripDemerits() {
    ...
    let keys = Object.keys(this.sharedData.class3Pretest);
    let uncheckedCount = 0;

    for (let keyIdx=2; keyIdx < keys.length; keyIdx++) {
      uncheckedCount += this.sharedData.class3Pretest[keys[keyIdx]] == false ? 1 : 0;
    }
  ...
  getAirBrakeDemerits() {
    ...
    let keys = Object.keys(this.sharedData.class3Pretest.airBrake);
    let uncheckedCount = 0;

    for (let keyIdx=0; keyIdx < keys.length; keyIdx++) {
      uncheckedCount += this.sharedData.class3Pretest.airBrake[keys[keyIdx]] == false ? 1 : 0;
    }
  ...
  finalizePretrip() {
    ...
    if (this.sharedData.class3Pretest.complete) {
      ...
      if (airPretripPassed && pretripPassed) {
        ...
        this.sharedData.class3Pretest.passed = true;
